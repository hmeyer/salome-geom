...
Standard_Integer GEOMImpl_PipeDriver::Execute(TFunction_Logbook& log) const
{
  ...
  if (aType == PIPE_BASE_PATH) {
    ...
    //aBuilder.SetMode(Standard_False);
    aBuilder.SetTransitionMode (BRepBuilderAPI_RightCorner);
    ...
    {
        NeedCreateSolid = Standard_False;
        aBuilder.Add (aShapeBase, Standard_False, Standard_True);
    }
    else if (aTypeBase == TopAbs_FACE)
    {
        NeedCreateSolid = Standard_True;
        ...
        {
            TopoDS_Shape aWireProf = aExpW.Current();
            aBuilder.Add (aWireProf, Standard_False, Standard_True);
        }
    }
    else if (aTypeBase == TopAbs_EDGE)
    {
  ...
  //building a rigid pipe
  else if (aType == PIPE_RIGID) {
    Handle(GEOM_Function) aRefBase = aCI->GetBase();
    TopoDS_Shape aShapeBase;
    ...
    aBuilder.SetTransitionMode (aTransition);
    ...
    if(aTypeBase == TopAbs_WIRE) {
      ...
      TopExp::MapShapes(aShapeBase, TopAbs_EDGE, aMapEdges);
      BRepAdaptor_Curve curve( TopoDS::Edge( aMapEdges.FindKey( 1 ) ) );
      curve.D1( curve.FirstParameter(), pos, tan );
      gp_Ax2 ax;
      ax.SetLocation( pos );
      ax.SetDirection( tan );
      aBuilder.SetMode( ax );
      aBuilder.Add (TopoDS::Wire( aShapeBase ));
    }
      ...
      gp_Pnt pos;
      gp_Vec tanU, tanV;
      BRepAdaptor_Surface surface( TopoDS::Face( aShapeBase ) );
      surface.D1( surface.FirstUParameter(), surface.FirstVParameter(), pos, tanU, tanV );
      gp_Ax2 ax;
      ax.SetLocation( pos );
      ax.SetDirection( tanU );
      aBuilder.SetMode( ax );
      TopExp_Explorer aExpW(aShapeBase,TopAbs_WIRE);
      for( ; aExpW.More(); aExpW.Next()) {
        TopoDS_Shape aWireProf = aExpW.Current();
        aBuilder.Add (aWireProf);
      }
    }
      ...
      gp_Pnt pos;
      gp_Vec tan;
      BRepAdaptor_Curve curve( TopoDS::Edge( aShapeBase ) );
      curve.D1( curve.FirstParameter(), pos, tan );
      gp_Ax2 ax;
      ax.SetLocation( pos );
      ax.SetDirection( tan );
      ...
      aBuilder.SetMode( ax );
      TopoDS_Shape aWireProf = BRepBuilderAPI_MakeWire( TopoDS::Edge( aShapeBase ) );
      aBuilder.Add (aWireProf);
    }
    else {
      Standard_ConstructionError::Raise("PIPE_RIGID not implemented for this shape type");
    }
    //aBuilder.Add (aShapeBase,Standard_False,Standard_True);
    aBuilder.Build();
